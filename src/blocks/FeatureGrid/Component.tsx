  ...
  return (
    <div className="container my-16">
      <header className="max-w-2xl">
        ...
        {intro ? (
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-slate-600 md:text-[15px] md:leading-7">
            {intro}
          </p>
        ) : null}
      </header>
      ...
      <ul className="mt-10 grid list-none gap-4 p-0 md:mt-12 md:grid-cols-2 md:gap-5">
        {cards.map((row, index) => (
          <li
            className="flex gap-4 rounded-lg border border-slate-200/90 bg-white p-4 shadow-[0_8px_30px_-12px_rgba(15,23,42,0.08)] md:gap-5 md:p-5"
            key={row.id ?? `${index}-${row.body?.slice(0, 24)}`}
          >
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-md bg-[#2eb0c1] md:h-12 md:w-12">
              <ShieldCheck aria-hidden className="size-5 text-white md:size-6" strokeWidth={2} />
            </div>
            <p className="min-w-0 text-sm leading-relaxed text-slate-600 md:text-[15px] md:leading-7">
              {row.body}
            </p>
          </li>
        ))}
